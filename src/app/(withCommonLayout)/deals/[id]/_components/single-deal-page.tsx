...
import { useGetDiscountedTiresByBrand, useGetDiscountedWheelsByBrand, useGetSingleDeal } from "@/src/hooks/deals.hook";
...
const SingleDealsPage = ({ params }: { params: { id: string } }) => {
  ...
  const {user} = useUser();
  console.log({data}, 'single_deal')
  return (
    <div className="container mx-auto px-4 py-8">
        {
            user?.email ? (
                <div>
                    <TireDealsSection brand={{...brand, discountPercentage, dealId: _id}} />
                    <WheelDealsSection brand={{...brand, discountPercentage, dealId: _id}} />
                </div>
            ) : (
                <DealLoginPage dealData={deal} />
            )
        }

...
import { useGetDiscountedWheelsByBrand, useGetSingleDeal } from '@/src/hooks/deals.hook';
...
const DealAllWheels = ({ params }: { params: { id: string } }) => {
    const { data: deal } = useGetSingleDeal(params.id);
          const {brand, discountPercentage, _id} = deal?.data || {};
  const queryClient = useQueryClient();
    const {data, isLoading, isError} = useGetDiscountedWheelsByBrand(brand?._id);
        if (isLoading) {
          return (
        ...
        if (isError) {
          return (
            <div className="flex h-[50vh] flex-col items-center justify-center gap-4">
              <p className="text-xl font-semibold text-destructive">Failed to load deal wheels</p>
              <Button onPress={() => queryClient.invalidateQueries({ queryKey: ["GET_DISCOUNTED_WHEELS_BY_BRAND"] })}>Try Again</Button>
            </div>
          )
        }
        ...
        <div className="container mx-auto px-4">
            ...
            <>
                ...
                {tireData.map((product: any) => (
                    <WheelCard key={product._id} product={product} />
                ))}
                </div>
            </>
            ) : (
            <div className="flex flex-col items-center justify-center py-20 text-center border rounded-lg">
                ...
                Currently, there are no wheel deals available for {brand?.name?.toUpperCase()}.
                </p>

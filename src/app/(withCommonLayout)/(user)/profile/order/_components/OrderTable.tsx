...
export default function OrderTable({ orders }: any) {
  ...
  const renderCell = (order: any, columnKey: any) => {
    ...
    switch (columnKey) {
      ...
      case "totalPrice":
        return <span className="font-semibold">${order.totalPrice?.toFixed(2)}</span>
      ...
      case "paymentStatus":
        return <StatusBadge status={order.payment?.paymentStatus} type="payment" />

      case "paymentMethod":
        return <span className="capitalize">{order.payment?.paymentMethod?.replace("_", " ")}</span>
